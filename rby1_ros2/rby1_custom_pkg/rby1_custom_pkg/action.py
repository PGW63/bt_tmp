...
class SemanticNavigator(LifecycleNode):
    """our lifecycle nav2 goal_pose node"""
    
    def __init__(self):
        super().__init__("semantic_navigator3")
        self.sub_ = None
        self.pub_ = None
        self.ser_ = None
        self.cli_ = None
        ...
        self.get_logger().info("Semantic Navigator (unconfigured).")
    ...
    def on_configure(self, state: State) -> TransitionCallbackReturn:
        ...
        self.sub_ = self.create_subscription(PoseWithCovarianceStamped,
                                                "/amcl_pose",
                                                    self.amcl_callback,
                                                    self.qos_profile)

        self.get_logger().info("Semantic Navigator (configured)")
        return TransitionCallbackReturn.SUCCESS
        
    def on_activate(self, state: State) -> TransitionCallbackReturn:
        self.get_logger().info("Semantic Navigator Activating...")
        # try:
            
        #     self.get_logger().info("Subscription is created")
        # except Exception as e:
        #     self.get_logger().error(f"Activate Failed! -> subsription problem")
        ...
        try:
            ...
            self.get_logger().info("Service is created")
            return TransitionCallbackReturn.SUCCESS
        except Exception as e:
            self.get_logger().error(f"Activate Failed! -> service problem")
            return TransitionCallbackReturn.FAILURE
    
    def on_deactivate(self, state: State) -> TransitionCallbackReturn:
        self.get_logger().info("Semantic Nav Deactivating...")
        if self.sub_ is not None:
            self.destroy_subscription(self.sub_)
            self.sub_ = None
        
        if self.ser_ is not None:
            self.destroy_service(self.ser_)
            self.ser_ = None
        
        self.get_logger().info("Sub and Service is stopped")
        return TransitionCallbackReturn.SUCCESS
    
    def on_cleanup(self, state: State)-> TransitionCallbackReturn:
        self.get_logger().info("Semantic Nav Cleaning...")
        self.sub_ = None
        self.ser_ = None

        self.get_logger().info("Semantic Nav cleaned up")
        return TransitionCallbackReturn.SUCCESS
    
    def on_shutdown(self, state: State) -> TransitionCallbackReturn:
        self.get_logger().info("Semantic Nav shuttind down...")
    ...
    def nav_callback(self, request, response):
        ...
        self.get_logger().info(f"Recieved {region_name}")
